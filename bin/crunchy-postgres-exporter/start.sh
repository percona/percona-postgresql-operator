...
set_default_postgres_exporter_env() {
	if [[ ! -v POSTGRES_EXPORTER_PORT ]]; then
...
if [[ ! ${#default_exporter_env_vars[@]} -eq 0 ]]; then
...
if [[ -f /conf/queries.yml ]]; then
	...
	for query in "${QUERIES[@]}"; do
		if [[ -f ${CONFIG_DIR?}/${query?}.yml ]]; then
	...
	if ((${VERSION?} >= 90600)) && ((${VERSION?} < 100000)); then
		if [[ -f ${CONFIG_DIR?}/pg96/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg96/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
	elif ((${VERSION?} >= 100000)) && ((${VERSION?} < 110000)); then
		if [[ -f ${CONFIG_DIR?}/pg10/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg10/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
		if [[ -f ${CONFIG_DIR?}/pg10/queries_pg_stat_statements.yml ]]; then
	...
	elif ((${VERSION?} >= 110000)) && ((${VERSION?} < 120000)); then
		if [[ -f ${CONFIG_DIR?}/pg11/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg11/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
		if [[ -f ${CONFIG_DIR?}/pg11/queries_pg_stat_statements.yml ]]; then
			cat ${CONFIG_DIR?}/pg11/queries_pg_stat_statements.yml >>/tmp/queries.yml
		else
			echo_warn "Query file queries_pg_stat_statements.yml not loaded."
		fi
	elif ((${VERSION?} >= 120000)) && ((${VERSION?} < 130000)); then
		if [[ -f ${CONFIG_DIR?}/pg12/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg12/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
		if [[ -f ${CONFIG_DIR?}/pg12/queries_pg_stat_statements.yml ]]; then
		...
		if [[ -f ${CONFIG_DIR?}/pg12/queries_pg_stat_statements_reset_info.yml ]]; then
	...
	elif ((${VERSION?} >= 130000)) && ((${VERSION?} < 140000)); then
		if [[ -f ${CONFIG_DIR?}/pg13/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg13/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
		if [[ -f ${CONFIG_DIR?}/pg13/queries_pg_stat_statements.yml ]]; then
		...
		# queries_pg_stat_statements_reset is only available in PG12+. This may
		# need to be updated based on a new path
		if [[ -f ${CONFIG_DIR?}/pg13/queries_pg_stat_statements_reset_info.yml ]]; then
			cat ${CONFIG_DIR?}/pg13/queries_pg_stat_statements_reset_info.yml >>/tmp/queries.yml
		else
			echo_warn "Query file queries_pg_stat_statements_reset_info.yml not loaded."
		fi
	elif ((${VERSION?} >= 140000)); then
		if [[ -f ${CONFIG_DIR?}/pg14/queries_general.yml ]]; then
			cat ${CONFIG_DIR?}/pg14/queries_general.yml >>/tmp/queries.yml
		else
			echo_err "Query file queries_general.yml does not exist (it should).."
		fi
		if [[ -f ${CONFIG_DIR?}/pg14/queries_pg_stat_statements.yml ]]; then
		...
		# queries_pg_stat_statements_reset is only available in PG12+. This may
		# need to be updated based on a new path
		if [[ -f ${CONFIG_DIR?}/pg14/queries_pg_stat_statements_reset_info.yml ]]; then
