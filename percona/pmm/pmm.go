...
// Container evaluates the pmm secret and configures the PMM3 container if the
// pmm token exists.
func Container(secret *corev1.Secret, pgc *v2.PerconaPGCluster) (corev1.Container, error) {
	...
	if v, exists := secret.Data[secretToken]; exists && len(v) != 0 {
		return sidecarContainer(pgc), nil
	}
...
func pmmConfigFile(pgc *v2.PerconaPGCluster) string {
...
// sidecarContainer refers to the construction of the PMM3 container.
...
func agentPrerunScript(pgc *v2.PerconaPGCluster) string {
