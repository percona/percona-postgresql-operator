...
func TestValidatePatroniDCS(t *testing.T) {
	// CEL-level validations (mixed schemes, items:Pattern, immutability) are
	// enforced at admission and require an envtest API server; they are not
	// unit-testable here.

	t.Run("nil patroni spec returns nil", func(t *testing.T) {
		cr := &PerconaPGCluster{}
		require.NoError(t, cr.ValidatePatroniDCS())
	})
	...
	t.Run("etcd DCS with nil etcd field returns error", func(t *testing.T) {
		cr := &PerconaPGCluster{
			Spec: PerconaPGClusterSpec{
				Patroni: &crunchyv1beta1.PatroniSpec{
					DCS: &crunchyv1beta1.PatroniDCS{
		...
		err := cr.ValidatePatroniDCS()
		require.Error(t, err)
		assert.Contains(t, err.Error(), "endpoints must be non-empty")
	})

	t.Run("etcd DCS with empty endpoints returns error", func(t *testing.T) {
		cr := &PerconaPGCluster{
			Spec: PerconaPGClusterSpec{
				Patroni: &crunchyv1beta1.PatroniSpec{
					DCS: &crunchyv1beta1.PatroniDCS{
	...
	t.Run("etcd DCS with one endpoint returns nil", func(t *testing.T) {
		cr := &PerconaPGCluster{
			Spec: PerconaPGClusterSpec{
				Patroni: &crunchyv1beta1.PatroniSpec{
					DCS: &crunchyv1beta1.PatroniDCS{
