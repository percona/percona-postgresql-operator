...
func TestBashPermissions(t *testing.T) {
	...
	if output, err := exec.CommandContext(ctx, "stat", "--help").CombinedOutput(); err != nil {
...
func TestBashRecreateDirectory(t *testing.T) {
	...
	stat := func(args ...string) string {
		cmd := exec.CommandContext(ctx, "stat", "-c", "%i %#a %N")
...
func TestBashSafeLink(t *testing.T) {
	...
	if output, err := exec.CommandContext(ctx, "mv", "--help").CombinedOutput(); err != nil {
